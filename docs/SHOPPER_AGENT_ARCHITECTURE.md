...
| Weather (optional, off by default) | Place/date request validation, one provider adapter, normalized daily forecast evidence, sanitized typed failures, and the agent tool granted by `destination-weather` | Using the saved ZIP on its own, persistence of its own, or a public API |
...
Weather is an optional `get_weather_forecast_tool` backed by a provider-neutral
client contract. When `weather.enabled` is true it is supplied to
`create_deep_agent` and granted only by the `destination-weather` skill, whose
forecast rules reach only a model request granted the tool. The server accepts
only a place the shopper named in the conversation, so the saved ZIP is never
looked up on its own. Disabled, the tool is not registered, and startup and
health checks make no weather request.
...
| [`outfit-styling`](../chain_server/skills/shopper/outfit-styling/SKILL.md) | `primary`, `product_procedure` | Build, complete, compare, balance, or refine a look | `search_catalog_tool`, `get_product_details_tool`, `check_product_availability_tool`, `check_active_promotions_tool`, `resolve_conversation_products_tool`, `get_weather_forecast_tool` |
...
| Weather (registered only when enabled) | `get_weather_forecast_tool` | Provider-neutral client; Visual Crossing is the first adapter |

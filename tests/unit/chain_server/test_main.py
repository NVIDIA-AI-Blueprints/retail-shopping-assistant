...
class TestDeepAgentsRuntimeRefs:
    ...
    def test_an_ambiguous_reference_is_clarified_never_searched(
        self,
        base_config,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """The order is: resolvable, then clarify, then search.

        A reference that matches more than one product the shopper was shown is
        answerable by asking which one. Searching the catalog there would offer
        new products to a shopper who was pointing at ones they had already
        seen, and would spend a retrieval to do it. The catalog is the last
        resort, not an alternative to asking.
        """
                ...
                results=[
                    ProductReferenceResolution(
                        ...
                        matches=[
                            ...
                            for position, product in enumerate((one, two), start=1)
                        ],
                        match_count=2,
                    )
                ]
        ...
        runtime._create_agent(
            State(user_id=111, query="add the black one in a 2"), identity
        )
        resolver = {fn.__name__: fn for fn in captured["tools"]}[
            "resolve_conversation_products_tool"
        ]
        ...
        assert "Black Satin Lace-Up Dress" in response

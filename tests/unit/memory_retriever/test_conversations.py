...
class _StrictPreV4CurrentWeatherScope(BaseModel):
    """The exact strict field boundary used by the rollback memory binary."""
...
def test_turn_start_negotiates_v2_and_future_requests_to_server_max(
    ...
    assert future.json()["contract_version"] == 6
...
def _finalize_turn(
    ...
    *,
    ...
    current_weather_scope_resolution: dict | None = None,
):
    payload = {
        "request_id": request_id,
    ...
    if current_weather_scope_resolution is not None:
        payload["current_weather_scope_resolution"] = (
...
def _weather_scope_resolution(
    *,
    ...
    preserve_pending_source_turn_id: str | None = None,
    complete_pending_source_turn_id: str | None = None,
    decline_pending_source_turn_id: str | None = None,
    supersede_pending_source_turn_id: str | None = None,
) -> dict:
    resolution: dict = {
        "expected_projection_version": expected_projection_version,
    ...
    if saved_area:
        resolution["location_scope"] = {"kind": "confirmed_saved_zip"}
    elif location is not None:
        resolution["location_scope"] = {
            "kind": "shopper_provided_location",
            "location": location,
        }
        if location_query is not None:
            resolution["location_scope"]["location_query"] = location_query
    if forecast_date is not None:
        resolution["requested_window"] = {
            "start_date": forecast_date.isoformat(),
            "end_date": forecast_date.isoformat(),
        }
    if pending_question is not None:
        resolution["pending_question"] = pending_question
    if preserve_pending_source_turn_id is not None:
        resolution["preserve_pending_source_turn_id"] = (
            preserve_pending_source_turn_id
        )
    if complete_pending_source_turn_id is not None:
        resolution["complete_pending_source_turn_id"] = (
            complete_pending_source_turn_id
        )
    if decline_pending_source_turn_id is not None:
        resolution["decline_pending_source_turn_id"] = (
            decline_pending_source_turn_id
        )
    if supersede_pending_source_turn_id is not None:
        resolution["supersede_pending_source_turn_id"] = (
...
def test_v4_weather_scope_resolution_preserves_retained_authority_provenance(
    conversation_db: TestClient,
) -> None:
    first = _start_turn(
        conversation_db,
        "conversation-weather-resolution",
        request_id="request-1",
        response_contract=4,
    ).json()
    assert (
    ...
    assert second["contract_version"] == 4
    assert (
        _finalize_turn(
            conversation_db,
            "conversation-weather-resolution",
    ...
    assert resolved["location"]["value"]["location"] == "NYC"
    assert resolved["location"]["source_turn_id"] == first["turn_id"]
    assert resolved["location"]["source_sequence"] == first["sequence"]
...
def test_v4_weather_scope_resolution_never_inherits_a_cleared_component(
    ...
    first = _start_turn(
        conversation_db,
        "conversation-weather-resolution-clear",
        request_id="request-1",
        response_contract=4,
    ).json()
    assert (
    ...
    assert resolved["window"]["value"]["start_date"] == "2026-08-15"
    assert resolved["pending_question"] == "event_location"
    assert resolved["pending_source_turn_id"] == second["turn_id"]
    assert resolved["pending_source_sequence"] == 2


def test_v4_pending_source_is_replaced_unless_exactly_preserved(
    conversation_db: TestClient,
) -> None:
    first = _start_turn(
        conversation_db,
        "conversation-weather-pending-source",
        request_id="request-1",
        response_contract=4,
    ).json()
    assert (
    ...
    replaced = third["projection"]["current_weather_scope"]
    assert replaced["pending_source_turn_id"] == second["turn_id"]
    assert replaced["pending_source_sequence"] == second["sequence"]
    wrong_preserve = _finalize_turn(
        conversation_db,
        "conversation-weather-pending-source",
        third["turn_id"],
        request_id="request-3",
    ...
    assert wrong_preserve.status_code == 409
    assert (
        wrong_preserve.json()["detail"]
    ...
    assert (
        _finalize_turn(
            ...
            current_weather_scope_resolution=_weather_scope_resolution(
                ...
                forecast_date=date(2026, 8, 23),
                pending_question="event_location",
                preserve_pending_source_turn_id=second["turn_id"],
            ),
        ).status_code
        == 200
    )

    preserved = _start_turn(
        conversation_db,
        "conversation-weather-pending-source",
        request_id="request-4",
        response_contract=4,
    ).json()["projection"]["current_weather_scope"]
    assert preserved["pending_source_turn_id"] == second["turn_id"]
    assert preserved["pending_source_sequence"] == second["sequence"]
    assert preserved["window"]["value"]["start_date"] == "2026-08-23"


def test_v4_exact_pending_completion_is_atomic_and_rotates_next_binding(
    conversation_db: TestClient,
) -> None:
    first = _start_turn(
        conversation_db,
        "conversation-weather-pending-completion",
        request_id="request-1",
        response_contract=4,
    ).json()
    assert (
    ...
    assert (
        _finalize_turn(
            ...
            current_weather_scope_resolution=_weather_scope_resolution(
                ...
                complete_pending_source_turn_id=first["turn_id"],
            ),
        ).status_code
        == 200
    )
    ...
    date_pending = third["projection"]["current_weather_scope"]
    assert date_pending["pending_question"] == "event_date"
    assert date_pending["pending_source_turn_id"] == second["turn_id"]
    assert date_pending["pending_source_sequence"] == second["sequence"]
    assert date_pending["location"]["value"]["location"] == "Seattle"

    missing_completion = _finalize_turn(
        conversation_db,
        "conversation-weather-pending-completion",
        third["turn_id"],
        request_id="request-3",
    ...
    assert missing_completion.status_code == 409
    assert (
        missing_completion.json()["detail"]
        == "current_weather_scope_resolution_conflict"
    )
    stale_completion = _finalize_turn(
        conversation_db,
        "conversation-weather-pending-completion",
        third["turn_id"],
        request_id="request-3",
    ...
    assert (
        _finalize_turn(
            ...
            current_weather_scope_resolution=_weather_scope_resolution(
                ...
                window_action="set",
                forecast_date=date(2026, 8, 15),
                complete_pending_source_turn_id=second["turn_id"],
            ),
        ).status_code
        == 200
    )

    completed = _start_turn(
        conversation_db,
        "conversation-weather-pending-completion",
        request_id="request-4",
        response_contract=4,
    ).json()["projection"]["current_weather_scope"]
    assert "pending_question" not in completed
    assert "pending_source_turn_id" not in completed
    assert completed["location"]["source_turn_id"] == second["turn_id"]
    assert completed["window"]["source_turn_id"] == third["turn_id"]
    assert completed["window"]["value"]["start_date"] == "2026-08-15"


def test_v6_exact_pending_decline_is_atomic_and_source_bound(
    conversation_db: TestClient,
) -> None:
    first = _start_turn(
        conversation_db,
        "conversation-weather-pending-decline",
        request_id="request-1",
        response_contract=6,
    ).json()
    assert first["contract_version"] == 6
    assert (
        _finalize_turn(
            conversation_db,
            "conversation-weather-pending-decline",
    ...
    stale_revision = _finalize_turn(
        ...
        current_weather_scope_resolution=_weather_scope_resolution(
            ...
            decline_pending_source_turn_id=first["turn_id"],
        ),
    )
    assert stale_revision.status_code == 409
    assert (
        stale_revision.json()["detail"]
        == "current_weather_scope_revision_conflict"
    )

    wrong_handle = _finalize_turn(
        conversation_db,
        "conversation-weather-pending-decline",
        second["turn_id"],
        request_id="request-2",
        ...
        current_weather_scope_resolution=_weather_scope_resolution(
            ...
            location_action="unavailable",
            window_action="retain",
            decline_pending_source_turn_id="another-turn",
        ),
    )
    assert wrong_handle.status_code == 409
    assert (
        wrong_handle.json()["detail"]
        == "current_weather_scope_resolution_conflict"
    )
    ...
    declined = v6_view["projection"]["current_weather_scope"]
    assert declined["revision"] == 2
    assert "pending_question" not in declined
    assert "pending_source_turn_id" not in declined
    assert "location" not in declined
    assert declined["location_unavailable"] == {
        "source_sequence": second["sequence"],
        "source_turn_id": second["turn_id"],
    }
    assert declined["window"]["source_turn_id"] == first["turn_id"]
    assert declined["window"]["value"]["start_date"] == "2026-08-15"
    assert [
        source["turn_id"]
        for source in v6_view["current_weather_scope_source_turns"]
    ] == [first["turn_id"], second["turn_id"]]
    with memory_main.SessionLocal() as db:
        projection = db.query(memory_main.ConversationProjection).one()
        stored_core = json.loads(projection.current_weather_scope_json)
        stored_unavailable = json.loads(
            projection.current_weather_unavailable_json
        )
    assert set(stored_core) == {"revision", "window"}
    assert stored_unavailable == {
    ...
    v5_scope = v5_view["projection"]["current_weather_scope"]
    assert "location_unavailable" not in v5_scope
    assert "window_unavailable" not in v5_scope
    assert [
        source["turn_id"]
        for source in v5_view["current_weather_scope_source_turns"]
    ] == [first["turn_id"]]


@pytest.mark.parametrize("pending_component", ["location", "window"])
def test_v6_new_subject_supersedes_exact_pending_binding(
    conversation_db: TestClient,
    pending_component: str,
) -> None:
    conversation_id = f"conversation-weather-supersede-{pending_component}"
    first = _start_turn(
        conversation_db,
        conversation_id,
        request_id="request-1",
    ...
    old_subject = (
        ...
        if pending_component == "location"
        else _weather_scope_resolution(
            expected_projection_version=first["projection"]["version"],
            expected_scope_revision=0,
            location_action="set",
            window_action="clear",
    ...
    assert (
        _finalize_turn(
            ...
            request_id="request-1",
            attempt_id=first["attempt_id"],
            current_weather_scope_resolution=old_subject,
        ).status_code
        == 200
    )
    ...
    new_subject = (
        _weather_scope_resolution(
            ...
            supersede_pending_source_turn_id=first["turn_id"],
        )
        if pending_component == "location"
        else _weather_scope_resolution(
            expected_projection_version=second["projection"]["version"],
    ...
    wrong_handle = dict(new_subject)
    wrong_handle["supersede_pending_source_turn_id"] = "wrong-turn"
    rejected = _finalize_turn(
        conversation_db,
        conversation_id,
        second["turn_id"],
        request_id="request-2",
        attempt_id=second["attempt_id"],
        current_weather_scope_resolution=wrong_handle,
    )
    ...
    retained_rejected = _finalize_turn(
        conversation_db,
        conversation_id,
        second["turn_id"],
        request_id="request-2",
        attempt_id=second["attempt_id"],
        current_weather_scope_resolution=retained_old_subject,
    )
    assert retained_rejected.status_code == 409
    assert (
        retained_rejected.json()["detail"]
        == "current_weather_scope_resolution_conflict"
    )
    assert (
        _finalize_turn(
            ...
            request_id="request-2",
            attempt_id=second["attempt_id"],
            current_weather_scope_resolution=new_subject,
        ).status_code
        == 200
    )
    ...
    assert "pending_question" not in current
    marker = current[f"{pending_component}_unavailable"]
    assert marker["source_turn_id"] == second["turn_id"]


@pytest.mark.parametrize("unavailable_component", ["location", "window"])
def test_v6_unavailable_component_can_be_retained_at_http_boundary(
    conversation_db: TestClient,
    unavailable_component: str,
) -> None:
    conversation_id = f"conversation-weather-{unavailable_component}-unavailable"
    first = _start_turn(
        conversation_db,
        conversation_id,
        request_id="request-1",
    ...
    initial_resolution = (
        ...
        if unavailable_component == "location"
        else _weather_scope_resolution(
            expected_projection_version=first["projection"]["version"],
            expected_scope_revision=0,
            location_action="set",
    ...
    assert (
        _finalize_turn(
            ...
            request_id="request-1",
            attempt_id=first["attempt_id"],
            current_weather_scope_resolution=initial_resolution,
        ).status_code
        == 200
    )
    ...
    assert (
        _finalize_turn(
            ...
            request_id="request-2",
            attempt_id=second["attempt_id"],
            current_weather_scope_resolution=retained_resolution,
        ).status_code
        == 200
    )
    ...
    marker = current[f"{unavailable_component}_unavailable"]
    assert marker == {
        "source_sequence": first["sequence"],
        "source_turn_id": first["turn_id"],
    }
    if unavailable_component == "location":
        assert current["window"]["source_turn_id"] == second["turn_id"]
        assert current["window"]["value"]["start_date"] == "2026-08-16"
    else:
        assert current["location"]["source_turn_id"] == second["turn_id"]
        assert current["location"]["value"]["location"] == "Portland"


@pytest.mark.parametrize(
    ("location_action", "window_action", "pending_question"),
    [
        ("unavailable", "clear", "event_date"),
        ("clear", "unavailable", "event_location"),
    ],
)
def test_v6_unavailable_component_rejects_any_pending_question(
    conversation_db: TestClient,
    location_action: str,
    window_action: str,
    ...
    started = _start_turn(
        conversation_db,
        f"conversation-weather-{pending_question}-unavailable-conflict",
        request_id="request-1",
        response_contract=6,
    ).json()
    rejected = _finalize_turn(
...
def test_v6_stale_unavailable_lane_is_inert_after_rollback_core_mutation(
    conversation_db: TestClient,
) -> None:
    conversation_id = "conversation-weather-unavailable-rollback"
    first = _start_turn(
        conversation_db,
        conversation_id,
        request_id="request-1",
    ...
    with memory_main.SessionLocal() as db:
        projection = db.query(memory_main.ConversationProjection).one()
        stale_unavailable = json.loads(
            projection.current_weather_unavailable_json
        )
        rollback_core = json.loads(projection.current_weather_scope_json)
        rollback_core.update(
            {
                "revision": 2,
                "location": {
                    "value": {
        ...
        projection.current_weather_scope_json = json.dumps(
            rollback_core,
            sort_keys=True,
            separators=(",", ":"),
        )
    ...
    assert stale_unavailable["scope_revision"] == 1
    resumed = _start_turn(
        conversation_db,
        conversation_id,
        request_id="request-3",
    ...
    scope = resumed["projection"]["current_weather_scope"]
    assert scope["revision"] == 2
    assert scope["location"]["value"]["location"] == "Seattle"
    assert "location_unavailable" not in scope
    assert [
        source["turn_id"]
        for source in resumed["current_weather_scope_source_turns"]
    ] == [first["turn_id"], second["turn_id"]]


def test_v4_pending_binding_keeps_strict_v3_scope_storage_rollback_safe(
    conversation_db: TestClient,
) -> None:
    first = _start_turn(
        conversation_db,
        "conversation-weather-pending-rollback",
        request_id="request-1",
        response_contract=4,
    ).json()
    assert (
    ...
    with memory_main.SessionLocal() as db:
        projection = db.query(memory_main.ConversationProjection).one()
        core_json = projection.current_weather_scope_json
        pending_payload = json.loads(
            projection.current_weather_pending_json
        )

    rollback_scope = _StrictPreV4CurrentWeatherScope.model_validate_json(
        core_json
    )
    assert set(json.loads(core_json)) == {"revision", "window"}
    assert pending_payload == {
        "pending_question": "event_location",
        "pending_source_sequence": 1,
        "pending_source_turn_id": first["turn_id"],
        "scope_revision": 1,
    }

    legacy_contract = _start_turn(
        conversation_db,
        "conversation-weather-pending-rollback",
        request_id="request-2",
        response_contract=3,
    ).json()
    assert legacy_contract["contract_version"] == 3
    assert set(
        legacy_contract["projection"]["current_weather_scope"]
    ) == {"revision", "window"}
    assert (
        _finalize_turn(
            conversation_db,
            "conversation-weather-pending-rollback",
            legacy_contract["turn_id"],
            request_id="request-2",
            attempt_id=legacy_contract["attempt_id"],
        ).status_code
        == 200
    )
    ...
    assert current["projection"]["current_weather_scope"][
        "pending_question"
    ] == "event_location"
    assert (
        _finalize_turn(
    ...
    rollback_payload = rollback_scope.model_dump(mode="json")
    rollback_payload.update(
        {
            "revision": rollback_scope.revision + 1,
            "location": {
                "value": {
                    "kind": "shopper_provided_location",
                    "location": "Seattle",
    ...
    rollback_scope = _StrictPreV4CurrentWeatherScope.model_validate(
        rollback_payload
    )
    with memory_main.SessionLocal() as db:
        projection = db.query(memory_main.ConversationProjection).one()
        projection.current_weather_scope_json = json.dumps(
            rollback_scope.model_dump(mode="json", exclude_none=True),
            sort_keys=True,
            separators=(",", ":"),
        )
        db.commit()
    ...
    assert resumed["location"]["value"]["location"] == "Seattle"
    assert "pending_question" not in resumed
    assert "pending_source_turn_id" not in resumed
    assert "pending_source_sequence" not in resumed


def test_v4_weather_scope_resolution_rejects_stale_revision_and_dual_update(
    conversation_db: TestClient,
) -> None:
    started = _start_turn(
        conversation_db,
        "conversation-weather-resolution-conflict",
        request_id="request-1",
        response_contract=4,
    ).json()
    resolution = _weather_scope_resolution(
        expected_projection_version=started["projection"]["version"],
        expected_scope_revision=9,
        location_action="set",
        window_action="clear",
        location="Seattle",
    )
    ...
    assert missing_retain.status_code == 409
    assert (
        missing_retain.json()["detail"]
        == "current_weather_scope_resolution_conflict"
    )
...
@pytest.mark.parametrize(
    ("embedded_pending", "expected_pending"),
    (
        pytest.param(
            {
                "pending_question": "event_location",
                "pending_source_turn_id": "turn-1",
                "pending_source_sequence": 1,
            },
            {
                "pending_question": "event_location",
            ...
            id="complete-source-bound-binding",
        ),
        pytest.param(
            {"pending_question": "event_location"},
            {},
            id="unsourceable-question-only-binding",
        ),
    ),
)
def test_pending_binding_migration_restores_strict_v3_scope_json(
    embedded_pending: dict,
    ...
    with upgrade_engine.begin() as connection:
        ...
        connection.execute(
            text(
                ...
                "current_weather_scope_json TEXT NOT NULL)"
            )
        )
        connection.execute(
            text(
                "INSERT INTO conversation_projection "
                "(conversation_id, current_weather_scope_json) "
                "VALUES ('conversation-1', :scope_json)"
            ),
            {
    ...
    memory_main.run_schema_migrations(upgrade_engine)

    with upgrade_engine.connect() as connection:
        core_json, pending_json, unavailable_json = connection.execute(
            text(
                "SELECT current_weather_scope_json, "
                "current_weather_pending_json, "
                "current_weather_unavailable_json "
    ...
    assert rollback_scope.revision == 1
    assert set(json.loads(core_json)) == {"revision", "window"}
    assert json.loads(pending_json) == expected_pending
    assert json.loads(unavailable_json) == {}

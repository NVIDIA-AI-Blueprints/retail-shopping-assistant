...
@pytest.mark.parametrize(
    ("projection", "source"),
    [
        (_projection().model_copy(update={"version": 7}), _source()),
        (
            _projection(),
            _source().model_copy(update={"after_sequence": 1}),
        ),
    ],
)
def test_work_rejects_stale_memory_boundaries(
...
@pytest.mark.asyncio
async def test_v1_turn_makes_no_compactor_call(
    base_config,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = runtime_mod.DeepAgentsRuntime(base_config)
    turn = _turn_start_result().model_copy(update={"contract_version": 1})
    monkeypatch.setattr(
        runtime,
        "_create_chat_model",
        lambda: (_ for _ in ()).throw(
            AssertionError("v1 memory cannot authorize summary compaction")
        ),
    )
...
async def test_turn_compacts_after_response_and_before_finalize(
    ...
    monkeypatch.setattr(runtime, "_start_conversation_turn", lambda *_args: turn)
...
@pytest.mark.asyncio
async def test_cancellation_during_compaction_finalizes_failed_without_advance(
    base_config,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = runtime_mod.DeepAgentsRuntime(base_config)
    ...
    async def execute(state, _identity):
        order.append("execute")
        state.response = "Completed response not yet finalized."
        state.product_results = [{"product_id": "dress-1"}]
        state.agent_diagnostics = {"final_termination_reason": "completed"}
        return state

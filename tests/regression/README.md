> **Superseded.** These twenty-four frozen turns were the first attempt at
> confirming a fix, and they matched on error strings in the reply -- which
> reported "no failed turns" over a size nobody asked for and again over a
> dress that had gone missing. Their turns now live as scenarios in
> `tests/evaluation/datasets/val/`, checked against the cart instead. Run
> `python -m tests.evaluation.src.replay --only J01,J02,J13`.

...
- `get_weather_forecast_tool` is registered read-only and granted only by
  ...
  prior candidates with a nonempty draft stay on ordinary grounding only when
  there is no current weather outcome. A comparison that calls only weather
  remains protected, while current non-weather business activity guarantees
  ordinary grounding. Saved mode
  ...
  retain only categorical `request_shape`, `location_source`,
  `provider_input`, and `outcome`, never a place, ZIP, date, resolved place,
  URL, body, or exception. Diagnostic string keys and values
  ...
  weather direction, only the accepted question, and the typed weather
  outcome/canonical block. Never ask for state, region, country, or a finer
  location solely because provider lookup failed.

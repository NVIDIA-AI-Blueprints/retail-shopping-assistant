...
not repeat an argument the validator rejected or weaken a preserved hard
constraint. Product meaning remains model-owned; the server validates only the
submitted structural values. If a valid advertised selection still cannot be
made, ask the shopper instead of claiming catalog absence."""
_SYNTHESIS_FALLBACK = (
...
class ToolLoopControlMiddleware(AgentMiddleware):
    ...
    def _observe_tool_results(self, messages: list[Any]) -> None:
        ...
        for result in messages[start:]:
            ...
            if SEARCH_SCOPE_COMPLETE_PREFIX in content:
                self._synthesis_required = True
            elif content.startswith(SEARCH_VALIDATION_ERROR_PREFIX):
    ...
    def _queue_repair(
        ...
        """Queue at most one structural catalog repair for the turn."""

        if self._repair_used or self._repair_pending or self._repair_in_flight:
            self._synthesis_required = True
            return
        native_validation_failure = _is_native_validation_failure(content)
        if _search_has_unadvertised_requirements(messages, tool_call_id):
            self._synthesis_required = True
            return
        ...
        self._repair_feedback = sanitized_feedback[:_REPAIR_FEEDBACK_LIMIT]

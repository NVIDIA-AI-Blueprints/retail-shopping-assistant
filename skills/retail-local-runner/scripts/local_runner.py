...
MODEL_ENV = RUN_DIR / "model-endpoints.env"
...
def base_env() -> dict[str, str]:
    env = os.environ.copy()
    model_env_loaded = load_model_env(env)
    default_key = env.get("NVIDIA_API_KEY", "").strip() or env.get("NGC_API_KEY", "").strip()
    if model_env_loaded and not default_key:
        default_key = "local-nim"
    if default_key:
...
def load_model_env(env: dict[str, str]) -> bool:
    if not MODEL_ENV.exists():
        return False
    for raw_line in MODEL_ENV.read_text().splitlines():
        line = raw_line.strip()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    configure_parser = subparsers.add_parser("configure", help="Write local model endpoint env for remote NIMs.")
    ...
    start_parser.add_argument("--nim-host", help="Remote NIM host URL. Rewrites local model endpoint env before starting.")

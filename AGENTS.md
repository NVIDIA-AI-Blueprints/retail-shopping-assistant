...
- `get_weather_forecast_tool` is registered read-only and granted only by
  ...
  an empty draft skips the protected weather-outcome decision editor; a scope
  resolution alone stays on ordinary grounding. The separate
  ...
  exact JSON with `venue_quote` set to a grounded quote or explicit `null`, plus
  one or two distinct allowlisted adjustment codes. A missing key or invalid
  non-null quote falls back; the server maps codes to fixed
